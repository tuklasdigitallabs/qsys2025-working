...
const ENV = {
  ...
  DB_URL:
    process.env.ADMIN_FIREBASE_DB_URL ||
...
app.post('/login', async (req, res) => {
  ...
  try {
    ...
    if (!['admin', 'superadmin'].includes(user.role)) {
      console.warn('[ADMIN-APP] User not admin:', emailFromAuth, 'role=', user.role);
    ...
    console.log('[ADMIN-APP] /login success, session user set:', req.session.user);
...
app.get('/branches/:id', requireAdmin, async (req, res) => {
  try {
    ...
    console.log('[ADMIN-APP] /branches/:id for', req.session.user.email, 'id=', id);
...
// ---------------- Start Server ----------------
app.listen(ENV.PORT, () => {
  console.log(`[ADMIN-APP] Listening on port ${ENV.PORT} (env: ${ENV.NODE_ENV})`);
});
